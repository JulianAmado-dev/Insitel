...
CREATE TABLE `intraNet_DB`.`areas_navegacion` (
  ...
  UNIQUE INDEX `nombre_item_navBar_UNIQUE` (`nombre_item_navBar` ASC) VISIBLE
) ENGINE=InnoDB COMMENT='Catálogo de items Que se van a traer desde la db para hacer la construcción del navBar';
...
CREATE TABLE `intraNet_DB`.`proyectos` (
  ...
  -- CONSTRAINT `fk_proyectos_area` FOREIGN KEY (`id_area`) REFERENCES `areas` (`id_area`) ON DELETE RESTRICT ON UPDATE CASCADE
...
CREATE TABLE `intraNet_DB`.`formulario_general` (
	...
	-- Info Requerimiento
    `tipo_proyecto` varchar (200) null,
    ...
    -- `aplica_proceso_compras` BOOLEAN DEFAULT FALSE,
    -- Timestamps
    `creado_en` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `actualizado_en` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id_proyecto`),
...
CREATE TABLE `intraNet_DB`.`proyecto_compras`(
  ...
  `descripcion` TEXT NULL,
  `cantidad` DECIMAL(10,2) NULL DEFAULT 1.00, -- Permitir decimales para unidades? O INT
...
CREATE TABLE `intraNet_DB`.`formulario_alcance` (
    ...
    `problema_necesidad` TEXT NULL,
    `entorno_actores` TEXT NULL,
